...
const OrderRouteDetail = () => {
  ...
  const { deliveryData, journeyInfo, handleGenerateQRCode, handleSendQRCode, handleCancel } = useOrderController()

  return (
    <Box className='container mx-auto p-4'>
      <Typography variant='h3' className='mb-6 font-semibold'>
        Configure route (Order ID: 012345678)
      </Typography>
              ...
              <Grid size={2.3}>
                <Box className='flex justify-end mt-8 px-4'>
                  <Box className='flex flex-col items-end space-y-4 w-60'>
                    <Button variant='contained' color='primary' size='large' onClick={handleGenerateQRCode} fullWidth>
                      Select
                    </Button>
                    <Button variant='outlined' color='primary' size='large' onClick={handleSendQRCode} fullWidth>
                      Request Change
                    </Button>
                    <Button variant='outlined' color='primary' size='large' onClick={handleCancel} fullWidth>
                      Cancel
                    </Button>
                  </Box>
                </Box>
