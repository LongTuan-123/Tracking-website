          ...
          <div className="p-6 bg-white rounded-lg shadow-md h-[459px]">
            <div className="flex justify-between items-center">
              <h3 className="text-xl font-bold">
                {orderInfo.orderId.label}{' '}
                <span className="font-normal">{orderInfo.orderId.value}</span>
              </h3>
              <p className="text-lg font-bold">
                {orderInfo.creationDate.label}{' '}
                <span className="font-normal">{orderInfo.creationDate.value}</span>
              </p>
            </div>
            ...
            <h3 className="text-xl font-bold">
              {orderInfo.product.name.label}{' '}
              <span className="font-normal">{orderInfo.product.name.value}</span>
            </h3>

            <p className="mt-4 font-bold">
              {orderInfo.product.ingredients.label}{' '}
              <span className="font-normal">
                {orderInfo.product.ingredients.value.join(', ')}
              </span>
            </p>
          ...
          <Grid item xs={12} md={3}>
            <div className="bg-white rounded-lg p-4">
              <p className="font-bold mb-2">
                {customerInfo.name.label}{' '}
                <br />
                <span className="font-normal">{customerInfo.name.value}</span>
              </p>
              <p className="font-bold mb-3">
                {customerInfo.phoneNumber.label}{' '}
                <br />
                <span className="font-normal">{customerInfo.phoneNumber.value}</span>
              </p>
              <p className="font-bold mb-3">
                ...
                <div key={index} className="flex items-center mb-12">
                  <div className="flex items-center">
                    ...
                    {item.mode.value ? (
                      <div className="flex items-center">
                        <img
                          src={`/${item.mode.value.toLowerCase()}.png`}
                          alt="icon"
                          className="w-5 h-5 mr-1"
                        />
                        <span
                          className={`text-${item.mode.color}-600 font-medium leading-tight`}
                        >
                          {item.mode.value}
                        </span>
                      </div>
